  ...
  const NodePtr & node, const NodePtr & goal, const NodeGetter & node_getter)
{
  ...
  _collision_checker->inCollision(
    goal->pose.x, goal->pose.y, goal->pose.theta /*bin number*/, _traverse_unknown);
  bool traverse_cost = d < 5.0;  // (_collision_checker->getCost() > 0.0 && d < 5.0) || d < 5.0);
  ...
  for (float i = 1; i < num_intervals; i++) {
    ...
    if (node_getter(index, next)) {
      ...
      if (next->isNodeValid(_traverse_unknown, _collision_checker, traverse_cost) && next != prev) {
