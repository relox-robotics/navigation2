...
#include "relox_costmap_2d/inflation_layer.hpp"
#include "tf2/utils.h"
...
inline double findCircumscribedCost(std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap)
{
  ...
  for (layer = costmap->getLayeredCostmap()->getPlugins()->begin();
    ...
    std::shared_ptr<relox_costmap_2d::InflationLayer> inflation_layer =
      std::dynamic_pointer_cast<relox_costmap_2d::InflationLayer>(*layer);
    if (!inflation_layer) {
      continue;
    }
